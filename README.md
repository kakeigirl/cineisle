...
- Web/PWA 新增「影院模式」：隐藏入口设置，影片使用完整视口，同时保留右侧聊天、笔记、卡片与影厅。
- 桌面端可拖动影片与侧栏之间的分隔线；影院模式里可把侧栏收成「AI 在这里」窄条，点击随时展开。
- 侧栏收起时，新聊天会在影片右上角短暂浮现，并显示未读数量；展开后可一键回到最新消息。
- 保留“上翻不乱跳”，同时避免中文输入法选词时按 Enter 误发送。
- PWA 静态资源改用网络优先策略，部署新版后不再长期停留在旧缓存。

## 本次更新：v0.4.7 场次存档与 Markdown 导出

- 一个房间对应一个独立观影场次，服务端会同时更新原子化 JSON 存档和可读的 Markdown 文档，重启后会从 JSON 自动恢复。
- 完整保留聊天/弹幕类型、发送者、ISO 时间戳、当时播放进度、时间轴笔记、观影卡和播放/暂停/跳转记录。
- Web/PWA 新增「导出本场 Markdown」；服务端导出失败时会自动回退到同一浏览器的本机场次缓存。
- 新增受 Token 保护的 `GET /api/rooms/:id/export.md` 和 MCP 工具 `export_room_markdown`。
- 服务端存档不会写入 Token，也不会持久保存截图的 base64 原图；房间读取和导出接口在配置 Token 后均要求鉴权。

### 生产环境持久磁盘

默认存档目录是 `server/data`，可通过 `CINEISLE_DATA_DIR` 修改。容器平台若使用临时文件系统，必须挂载持久磁盘并把该变量指向挂载目录，例如：

```text
CINEISLE_DATA_DIR=/data/cineisle
```

如果暂时没有持久磁盘，同一浏览器仍会保留每个房间的完整本机缓存，并可导出 Markdown；但更换浏览器或清理网站数据后，本机备份也会消失。
